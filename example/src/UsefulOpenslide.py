...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option("--input_folder", dest="input_folder",type="string",
                      help="input folder name")
    parser.add_option("--sample_names", dest="sample_names",type="string",
                      help="sample names")
    parser.add_option("--output", dest="output",type="string",
                      help="output folder")
    (options, args) = parser.parse_args()
    os.mkdir(options.output)
    files = glob(os.path.join(options.input_folder, "*.tiff"))
    for k, f in tqdm(enumerate(files)):
        try:
            image = GetWholeImage(f, level=5)
            imsave(os.path.join(options.output, options.sample_names + "__{:02d}__.png").format(k), image)
        except:
            print "failed on {}".format(os.path.basename(f))
            continue